...
public class AiNoteGenerator extends AiGeneratorBase {
    ...
    private List<ChatMessage> buildNoteMessages(String context, String requirements,
        ...
        String systemTemplate = """
                ...
                # 输出格式（仅输出 Markdown）
                # [动态标题规则]
                标题必须**精准反映本文档的核心主题**，而非泛泛的"学习笔记"。
                - 提炼文档中最具辨识度的关键词组合，不超过 15 字
                - 禁止使用"笔记""总结""概述"等无信息量的词
                - 示例：文档讲贝叶斯定理 → "贝叶斯定理：从条件概率到后验推断"；文档讲快速排序 → "快排的分治策略与复杂度分析"
                ...
                *(本部分是笔记的灵魂。拒绝罗列定义，必须讲透"为什么"和"怎么运作")*
                ...
                ## 知识可视化
                根据内容特征，选择最合适的可视化方式。**优先使用 Mermaid 渲染**，仅在 Mermaid 无法表达时使用占位符。

                ### 方式一：Mermaid 代码块（首选，前端会实时渲染图表）
                - **触发条件**：内容包含可渲染的流程、状态变化、层级分类或对象关系。
                - **图表类型**：流程图(graph)、状态图(stateDiagram-v2)、时序图(sequenceDiagram)、类图(classDiagram)。
                ...
                *(在此处输出 ```mermaid 代码块，并在图表前后各加一行说明文字)*
                ...
                ### 方式二：图表占位符（Mermaid 无法表达时的备选方案）
                当需要表达**时间线、概念地图、对比矩阵、层次树**等 Mermaid 不擅长的结构时，
                使用以下自定义代码块语法生成占位符，前端会渲染为专用可视化组件：

                ```chart-timeline
                描述：（简要说明该时间线展示什么内容，每行一个事件，格式：时期 → 事件描述）
                ```

                ```chart-concept
                描述：（简要说明概念关系图的核心节点和关系）
                ```

                ```chart-compare
                描述：（简要说明对比的维度和对象，用表格呈现差异）
                ```
                ...
                **选择指南**：
                - 有明确时间顺序的历史/发展/演化 → `chart-timeline`
                - 概念间有多对多关系 → `chart-concept`
                - 2-4 个方案/技术/方法横向对比 → `chart-compare`
                - 层级/分类/树形结构 → `chart-tree`
