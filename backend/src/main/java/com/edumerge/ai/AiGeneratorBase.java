...
public abstract class AiGeneratorBase {
    ...
    protected String buildSubjectRules(String subjectType) {
        ...
        return switch (subjectType) {
            case "ALGORITHM" -> """
                    ...
                    - 图解优先：涉及数据结构、算法流程、递归展开等内容时，尽量使用图表或步骤示意辅助说明
                    """;
            case "MATH" -> """
                    # 学科策略：数学
                    - 公式推导：要求推导或证明某个公式/定理
    ...
    /**
     * 根据学科类型生成笔记结构指引（区别于出题策略，聚焦知识理解与可视化）
     */
    protected String buildNoteSubjectGuide(String subjectType) {
        if (subjectType == null) subjectType = SubjectClassifier.GENERAL;
        return switch (subjectType) {
            case "ALGORITHM" -> """
                    # 学科笔记指引：算法与数据结构
                    - 侧重：算法思想、状态转移/递归推导、复杂度分析、边界条件
                    - 必须包含：核心代码逻辑的伪代码或步骤拆解、不同算法的优劣对比表格
                    - 图表建议：优先使用流程图展示算法执行分支，或使用状态图展示 DP 状态转移
                    """;
            case "MATH" -> """
                    # 学科笔记指引：数学
                    - 侧重：定理的几何/物理直觉、公式推导过程、适用前提与反例
                    - 必须包含：核心公式的变量含义拆解、定理间的逻辑依赖链
                    - 图表建议：优先使用象限图或坐标轴概念图辅助说明几何意义（若适用）
                    """;
            case "PROGRAMMING" -> """
                    # 学科笔记指引：程序设计
                    - 侧重：语法/API 的底层语义、设计模式的适用场景、常见 Bug 陷阱
                    - 必须包含：Best Practice 代码片段、反模式警告
                    - 图表建议：优先使用类图展示对象关系，或时序图展示交互流程
                    """;
            case "THEORY" -> """
                    # 学科笔记指引：计算机理论
                    - 侧重：系统机制的工作原理、状态流转、性能瓶颈与调优参数
                    - 必须包含：核心协议的握手/挥手/交互步骤、不同技术方案的选型对比
                    - 图表建议：必须使用状态图展示进程/连接状态机，或时序图展示网络交互
                    """;
            case "MEDICAL" -> """
                    # 学科笔记指引：医学
                    - 侧重：发病机制的因果链、临床表现的鉴别诊断、药物的作用靶点与不良反应
                    - 必须包含：相似疾病/症状的鉴别诊断表格、病理生理的演变过程
                    - 图表建议：优先使用流程图展示疾病发展路径或药物代谢途径
                    """;
            case "HUMANITIES" -> """
                    # 学科笔记指引：人文社科
                    - 侧重：理论的核心主张、历史背景、不同学派的争论与批判性思考
                    - 必须包含：理论在现实案例中的映射、核心概念的演进时间线
                    """;
            case "SCIENCE" -> """
                    # 学科笔记指引：自然科学
                    - 侧重：实验设计（变量控制）、微观机制、宏观现象的因果解释
                    - 必须包含：核心反应方程式/物理定律的适用条件、实验现象与结论的映射
                    """;
            default -> """
                    # 学科笔记指引：通用
                    - 侧重：核心概念的准确理解、逻辑框架的梳理、实际生活/工作中的应用
                    """;
        };
    }
